...
def check_launch_ref(server_cfg: dict, cap: str, ref: str, where: str) -> None:
    args = server_cfg.get("args")
    ...
    if expected_extra not in spec or not spec.endswith(f"@{ref}"):
        fail(f"{where}: --from must use [{cap}] and end in @{ref}, got {spec!r}")
